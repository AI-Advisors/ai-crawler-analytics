...
Each is about 50 lines, fire-and-forget, and never delays the crawler's response. The Express snippet needs Node 18 or newer (built-in fetch). The Cloudflare and Express snippets skip static-asset requests so all four platforms count page requests the same way. If you add your own rows to the tag's registry, mirror them in the snippet's prefilter regex too, or your additions will only be seen for events the container already receives. Configure three values in the snippet: your server container URL, the **same GA4 Measurement ID you set on the tag**, and the optional Ping secret (mirrored in the tag's Advanced settings, which makes spoofed pings much harder; the tag also ignores the bot_ua parameter on any event that is not an ai_crawler_ping). The snippet sends the **full** user agent to your own container; the tag classifies it and truncates only on output.

The ping travels as a standard GA4 `g/collect` request because that is the shape the GA4 client pre-installed in every server container claims, so there is nothing to add or configure in your container beyond the tag itself. A JSON POST to `/mp/collect` is **not** claimed by a default server container: we tested this on a live tagging server in August 2026, and the ping was dropped with the same 400 response a nonsense path gets. Earlier versions of these snippets used that shape; if you installed one, replace it.
